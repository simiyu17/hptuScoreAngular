...
export class DashboardService {
  ...
  getCountyAssessmentSummaryByPillar = (assessment: CountyAssessmentMetaData | null, pillarName: string | null): Observable<any> => {
    ...
    return this.httpClient.get(`${this.globalService.BASE_API_URL_V2}/report/county-assessments-summary?${queryParams}`);
  }
  ...
  exportCountyAssessmentSummaryToExcel(metaDataId?: number): Observable<any> {
    if(metaDataId){
      return this.httpClient.get(`${this.globalService.BASE_API_URL}/report/export-to-excel?metaDataId=${metaDataId}&analysisByPillarTableTitle=Summary`);
    }
    return of(null)
  ...
  exportCountyAssessmentSummaryToExcelV2(assessment?: CountyAssessmentMetaData): Observable<any> {
    if(!assessment){
      return this.httpClient.get(`${this.globalService.BASE_API_URL_V2}/report/county-assessments-summary-excel`);
    }
    return this.httpClient.get(`${this.globalService.BASE_API_URL_V2}/report/county-assessments-summary-excel?countyCode=${assessment?.countyCode}&assessmentQuarter=${assessment?.assessmentQuarter}&assessmentYear=${assessment?.assessmentYear}`);
  }
}
